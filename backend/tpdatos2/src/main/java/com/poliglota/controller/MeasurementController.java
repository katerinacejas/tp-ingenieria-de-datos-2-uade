...
public class MeasurementController {
    ...
    @PostMapping public ResponseEntity<Measurement> create(@RequestBody Measurement measurement){
        return ResponseEntity.ok(measurementService.save(measurement)); }
    @GetMapping("/{sensorId}") public ResponseEntity<List<Measurement>> getBySensor(@PathVariable String sensorId){
